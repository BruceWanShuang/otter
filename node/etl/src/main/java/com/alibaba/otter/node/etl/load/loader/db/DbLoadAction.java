...
public class DbLoadAction implements InitializingBean, DisposableBean {
    ...
    private void doDdl(DbLoadContext context, List<EventData> eventDatas) {
        for (final EventData data : eventDatas) {
            ...
            logger.warn("pt-online-schema-change load ddl eventData:"+data);
            try {
                ...
                dropPtonlineOldTable(dbDialect, data);
            } catch (Throwable e) {
                if (skipDdlException) {
                    // do skip
...
//    private boolean prePtonlineDDLCheck(DbDialect dbDialect, EventData data){
//        try{
//            String ddlSchemaName =  data.getDdlSchemaName();
//            String tableName =  data.getTableName();
//            if(!data.getEventType().isRename() && tableName.startsWith("ptonline_") && tableName.endsWith("_new")){
//                String newTable = tableName.substring(9, tableName.length()-4);
//                Table tableOld = DdlUtils.findTable(dbDialect.getJdbcTemplate(), ddlSchemaName, ddlSchemaName,
//                        newTable, null);
//                if(tableOld == null){
//                    logger.warn("ptonlineCheck skip sql:"+data.getSql());
//                    return false;
//                }
//            }
//        }catch (Exception e){
//            logger.warn("ptonlineCheck error", e);
//        }
//
//        return true;
//    }
    private void dropPtonlineOldTable(final DbDialect dbDialect, final EventData data){
        String dropSql ="";
        try{
            if(!data.getEventType().isRename()){
                return;
            }
            if(!data.getSql().contains("ptonline_") && !data.getSql().contains("_new")){
                return;
            }
            Boolean isDropPtonlineOldTable = false;
            List<DdlResult> results = DruidDdlParser.parse(data.getSql(), data.getDdlSchemaName());
            if(results != null && results.size() >= 1){
                for (DdlResult ddlResult : results) {
                    if(ddlResult.getOriTableName().startsWith("ptonline_")
                            && ddlResult.getOriTableName().endsWith("_new")){
                        isDropPtonlineOldTable = true;
                    }
                }
            }
            if(!isDropPtonlineOldTable){
                return;
            }
            ...
            if(results.size() == 2){
                for (DdlResult ddlResult : results) {
                    if(!ddlResult.getOriTableName().startsWith("ptonline_")
                            && !ddlResult.getOriTableName().endsWith("_new")){
                        oldSchemaName = ddlResult.getSchemaName();
                        oldTableName = ddlResult.getTableName();
                    }
                }
            }
            if(results.size() == 1){
            ...
            if(StringUtils.isBlank(oldTableName) || StringUtils.isBlank(oldSchemaName)){
                return;
            }
            dropSql = "DROP TABLE IF EXISTS `"+oldSchemaName+"`.`"+oldTableName+"`";
            final String dropSqlFinal = dropSql;
            dbDialect.getJdbcTemplate().execute(new StatementCallback<Boolean>() {
                public Boolean doInStatement(Statement stmt) throws SQLException, DataAccessException {
                    Boolean result = true;
                    if (dbDialect instanceof MysqlDialect && StringUtils.isNotEmpty(data.getDdlSchemaName())) {
                        // 如果mysql，执行ddl时，切换到在源库qq执行的schema上
                        // result &= stmt.execute("use " +
                        // data.getDdlSchemaName());
                    ...
                    result &= stmt.execute(dropSqlFinal);
                    return result;
                }
            });
            logger.warn("drop pt-online-schema-change old table sql:"+dropSql);
        }catch (Exception e){
            logger.warn("drop pt-online-schema-change old table error sql:"+dropSql, e);
        }
    }
    ...
    public static void main(String[] args){
        ...
        data.setEventType(EventType.RENAME);
        data.setDdlSchemaName("ics_core_car");
        data.setSql("RENAME TABLE `ics_core_car`.`driver_vehicle_test3` TO `ics_core_car`.`_driver_vehicle_test2_old`, `ics_core_car`.`ptonline_driver_vehicle_test2_new` TO `ics_core_car`.`driver_vehicle_test3`");
//        data.setSql("RENAME TABLE `ics_core_car`.`ptonline_driver_vehicle_test2_new` TO `ics_core_car`.`_driver_vehicle_test2_old`");

        Boolean result= true;
        result &= false;
        System.out.println( result);
//        DbLoadAction.dropPtonlineOldTable(null, data);
    }
}
