        ...
        this.tables = OtterMigrateMap.makeSoftValueComputingMap(new Function<List<String>, Table>() {
            ...
            public Table apply(List<String> names) {
                ...
                try {
                    ...
                    if(table ==null && names.get(1).startsWith("ptonline_") && names.get(1).endsWith("_new")){
                        String newTable = names.get(1).substring(9, names.get(1).length()-4);
                        Table tableOld = DdlUtils.findTable(jdbcTemplate, names.get(0), names.get(0), newTable, filter);
                        if(tableOld != null){
                            tableOld.setName(names.get(1));
                            table = tableOld;
                        }
                    }
                    if (table == null) {
