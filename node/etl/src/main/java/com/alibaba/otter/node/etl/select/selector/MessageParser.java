        ...
        if (StringUtils.equalsIgnoreCase(pipeline.getParameters().getSystemSchema(), schemaName)) {
            ...
            if (eventType.isDdl()) {
                ...
                if (ddlSync) {
                    ...
                    if(dataMedia == null && eventType.isRename()
                            && rowChange.getSql().contains("ptonline_") && rowChange.getSql().contains("_new")){
                        logger.warn("pt-online-schema-change parser rename ddl eventData:"+eventData);
                        List<DdlResult> results = DruidDdlParser.parse(rowChange.getSql(), schemaName);
                        if(results != null && results.size() == 2){
                            for (DdlResult result : results) {
                                if(result.getOriTableName().startsWith("ptonline_") && result.getOriTableName().endsWith("_new")){
                                    dataMedia = ConfigHelper.findSourceDataMedia(pipeline,
                                            schemaName, result.getTableName(), notExistReturnNull);
                                    break;
                                }
                            }
                            if(dataMedia == null){
                                String interTableName= null;
                                String ptonlineMidTable = null;
                                for (DdlResult result : results) {
                                    if(!result.getOriTableName().startsWith("ptonline_") && !result.getOriTableName().endsWith("_new")){
                                        interTableName = result.getTableName();
                                    }
                                    if(result.getOriTableName().startsWith("ptonline_") && result.getOriTableName().endsWith("_new")){
                                        ptonlineMidTable = result.getOriTableName();
                                        dataMedia = ConfigHelper.findSourceDataMedia(pipeline,
                                                schemaName, result.getOriTableName(), notExistReturnNull);
                                    }
                                }

                                if(dataMedia != null && StringUtils.isNotBlank(interTableName) && StringUtils.isNotBlank(ptonlineMidTable)){
                                    String newSql = "RENAME TABLE `"+schemaName+"`.`"+ptonlineMidTable+"`" + " TO `"+schemaName+"`.`"+interTableName+"`";
                                    eventData.setTableName(ptonlineMidTable);
                                    eventData.setSql(newSql);
                                }
                            }
                            logger.warn("pt-online-schema-change parser rename ddl new eventData:"+eventData);
                            if(dataMedia == null){
                                logger.warn("pt-online-schema-change parser rename ddl but dataMedia is null, eventData:"+eventData);
                                return null;
                            }
                        }
                    }
