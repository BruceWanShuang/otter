...
public class RowDataTransformer extends AbstractOtterTransformer<EventData, EventData> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RowDataTransformer.class);
    ...
    public EventData transform(EventData data, OtterTransformerContext context) {
        ...
        if (data.getEventType().isDdl()) {
            logger.warn("pt-online-schema-change transformer ddl eventData:"+data);
            // ddl不需要处理字段
            if (StringUtils.equalsIgnoreCase(result.getSchemaName(), data.getSchemaName())
                ...
                if(data.getEventType().isRename() && data.getSql().contains("ptonline_")
                        && data.getSql().contains("_new")){
                    List<DdlResult> results = DruidDdlParser.parse(data.getSql(), result.getSchemaName());
                    if(results != null && results.size() == 2){
                        for (DdlResult ddlResult : results) {
                            if(ddlResult.getOriTableName().startsWith("ptonline_") && ddlResult.getOriTableName().endsWith("_new")){
                                sql = data.getSql().replace("`"+ddlResult.getTableName()+"`",  "`"+result.getTableName()+"`");
                                sql = sql.replace("`"+data.getSchemaName()+"`.",  "`"+result.getSchemaName()+"`.");
                                break;
                            }
                        }
                    }
                ...
                result.setSql(sql);
                logger.warn("pt-online-schema-change transformer ddl result:"+result);
                return result;
        ...
        if (useTableTransform || enableCompatibleMissColumn) {// 控制一下是否需要反查table
            ...
            Table table = null;
            if(result.getTableName().startsWith("ptonline_") && result.getTableName().endsWith("_new")){
                try{
                    table = dbDialect.findTable(result.getSchemaName(), result.getTableName());
                }catch (Exception e){
                    logger.warn("pt-online-schema-change findTable error, result:"+result, e);
                    return null;
                }
            }else{
                table = dbDialect.findTable(result.getSchemaName(), result.getTableName());
    ...
    public static void main(String[] args){
        ...
        String sql = DdlUtils.convert("RENAME TABLE `ics_core_car`.`ptonline_driver_vehicle_test_new` TO `ics_core_car`.`_driver_vehicle_test_old`",
                "ics_core_car",
                "ptonline_driver_vehicle_test_new",
                "test",
                "ptonline_driver_vehicle_test_new");
